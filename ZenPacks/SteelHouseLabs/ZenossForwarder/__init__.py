...
import logging
import os
log = logging.getLogger('zen.zenforwarder')

import ZenPacks.SteelHouseLabs.ZenossForwarder
...
class ZenPack(ZenPackBase):
    ...
    def install(self, app):
        ...
        # Start daemon
        self.startDaemon(app.zport.dmd, daemonName='zenforwarder')
    ...
    def remove(self, app, leaveObjects=True):
        ...
        # Stop daemon
        self.stopDaemon(app.zport.dmd, daemonName='zenforwarder')
    ...
    def updateZenPackPersistance(self, dmd):
        ...
        dmd.zenPackPersistence.refreshCatalog(clear=0,pghandler=StdoutHandler())


    def symlinkPlugin(self):
        log.info('Linking daemon into $ZENHOME/bin')
        os.system('ln -sf %s %s' % (self.path('daemon/zenforwarder'), zenPath('bin', 'zenforwarder')))
        os.system('chmod a+x %s' % (zenPath('bin', 'zenforwarder')))
        os.system('touch %s' % (zenPath('etc', 'zenforwarder.conf')))


    def removePluginSymlink(self):
        log.info('Removing daemon from $ZENHOME/bin')
        os.system('rm -f %s' % (zenPath('bin', 'zenforwarder')))
